...
describe('azure-mobile-apps.data.sql.operations', function () {
    ...
    describe('update', function () {
        ...
        it('generates simple statement and parameters', function () {
            ...
            expect(statement.sql).to.equal('UPDATE [dbo].[table] SET [p1] = @p1 WHERE [id] = @id ; SELECT @@rowcount as __rowcount; SELECT * FROM [dbo].[table] WHERE [id] = @id');
    ...
    describe('updateSchema', function () {
        var updateSchema = require('../operations/updateSchema');

        it('generates simple statement', function () {
            var statement = updateSchema({ table: 'test' }, [{ name: 'id' }], { id: 1, text: 'test' });
            expect(statement.sql).to.equal('ALTER TABLE [dbo].[test] ADD [text] NVARCHAR(MAX) NULL')
        });
    });
});
