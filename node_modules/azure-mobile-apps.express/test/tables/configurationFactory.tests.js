...
describe('azure-mobile-apps.express.tables.configurationFactory', function () {
    it('generates specified middleware stack for overall execution', function () {
        ...
        expect(result[3]).to.equal(testMiddleware);
    });
    
    it('generates default middleware stack for overall execution', function () {
        var stack = configurationFactory()();
        expect(stack.length).to.equal(3);
        expect(stack[2].handle).to.equal(express.Router().handle);
        expect(stack[2].stack.length).to.equal(4);        
        expect(stack[2].stack[0].route.stack.length).to.equal(1);
    });
    ...
    it('generates middleware stack for individual operations', function () {
        ...
        var stack = factory();
        expect(stack[2].stack[0].route.stack.length).to.equal(2);
        expect(stack[2].stack[0].route.stack[0].handle).to.equal(testMiddleware);
        expect(stack[2].stack[0].route.stack[1].handle).to.equal(testMiddleware);
    });
    
    it('generates default middleware stack for individual operations', function () {
        var stack = configurationFactory()();
        expect(stack[2].stack[0].route.stack.length).to.equal(1);
        expect(stack[2].stack[0].route.stack[0].handle).to.equal(executeOperation);
    });
