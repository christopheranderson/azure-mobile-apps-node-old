...
describe('azure-mobile-apps.express.tables.middlewareFactory', function () {
    it('uses executeOperation for operation middleware if none was specified', function () {
        ...
        factory(configuration, router);

        expect(router.stack[0].route.stack.length).to.equal(1);
        expect(router.stack[0].route.stack[0].handle).to.equal(executeOperation);
    });
    ...
    it('returns router if no execution middleware was specified', function () {
        ...
        var results = factory(configuration, router);
        
        expect(results.length).to.equal(3);
        expect(results[2]).to.equal(router);
    })
    ...
    it('arranges middleware onto router', function () {
        ...
        expect(router.stack[0].route.stack.length).to.equal(1);
        expect(router.stack[0].route.stack[0].handle).to.equal(testMiddleware);

        expect(router.stack[1].route.methods.post).to.be.true;
        expect(router.stack[1].route.stack.length).to.equal(2);
        expect(router.stack[1].route.stack[0].handle).to.equal(testMiddleware);
