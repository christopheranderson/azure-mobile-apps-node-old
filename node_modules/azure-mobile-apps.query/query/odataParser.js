...
var ctor = function (expression) {
    ...
    this.it = new expressions.ParameterExpression();
...
var classMembers = {
    filter: function (predicate) {
        var parser = new ODataParser(predicate);
        var filter = parser.parse();
        return filter;
    },

    orderBy: function (ordering) {
        var parser = new ODataParser(ordering);
...
var instanceMembers = {
    ...
    _parseLogicalOr: function () {
        ...
        while (this.token.id == 'Or') {
            ...
            left = new expressions.BinaryExpression(left, right, 'Or');
        }
        return left;
    },
    ...
    _parseLogicalAnd: function () {
        ...
        while (this.token.id == 'And') {
            ...
            left = new expressions.BinaryExpression(left, right, 'And');
        }
        return left;
    },
    ...
    _parseComparison: function () {
        ...
        while (this.token.id == 'Equal' || this.token.id == 'NotEqual' || this.token.id == 'GreaterThan' ||
            ...
            switch (opId) {
                case 'Equal':
                    left = new expressions.BinaryExpression(left, right, 'Equal');
                    break;
                case 'NotEqual':
                    left = new expressions.BinaryExpression(left, right, 'NotEqual');
                    break;
                case 'GreaterThan':
                    left = new expressions.BinaryExpression(left, right, 'GreaterThan');
                    break;
                case 'GreaterThanEqual':
                    left = new expressions.BinaryExpression(left, right, 'GreaterThanOrEqual');
                    break;
                case 'LessThan':
                    left = new expressions.BinaryExpression(left, right, 'LessThan');
                    break;
                case 'LessThanEqual':
                    left = new expressions.BinaryExpression(left, right, 'LessThanOrEqual');
                    break;
            }
        }
        return left;
    ...
    _parseAdditive: function () {
        ...
        while (this.token.id == 'Add' || this.token.id == 'Sub') {
            ...
            switch (opId) {
                case 'Add':
                    left = new expressions.BinaryExpression(left, right, 'Add');
                    break;
                case 'Sub':
                    left = new expressions.BinaryExpression(left, right, 'Subtract');
                    break;
            }
        }
        return left;
    ...
    _parseMultiplicative: function () {
        ...
        while (this.token.id == 'Multiply' || this.token.id == 'Divide' ||
            ...
            switch (opId) {
                case 'Multiply':
                    left = new expressions.BinaryExpression(left, right, 'Multiply');
                    break;
                case 'Divide':
                    left = new expressions.BinaryExpression(left, right, 'Divide');
                    break;
                case 'Modulo':
                    left = new expressions.BinaryExpression(left, right, 'Modulo');
                    break;
            }
        }
        return left;
    ...
    _parseUnary: function () {
        if (this.token.id == 'Minus' || this.token.id == 'Not') {
            ...
            if (opId == 'Minus') {
                expr = new expressions.UnaryExpression(expr, 'Negate');
            } else {
                expr = new expressions.UnaryExpression(expr, 'Not');
            }
            return expr;
        }
        return this._parsePrimary();
    ...
    _parseMemberAccess: function (instance) {
        ...
        if (this.token.id == 'OpenParen') {
            ...
            return new expressions.MemberExpression(instance, id);
        }
    },
    ...
    _parseMappedFunction: function (mappedMember, errorPos) {
        ...
        if (mappedMember.isMethod) {
            // a mapped function
            return new expressions.FunctionCallExpression(instance, mappedMember, args);
        }
        else {
            // a mapped Property/Field
            return new expressions.MemberExpression(instance, mappedMember);
        }
    },
    ...
    _parseIntegerLiteral: function () {
        ...
        if (this.token.text.toUpperCase() == 'L') {
            ...
            this._nextToken();
            return new expressions.ConstantExpression(value);
        }
        return new expressions.ConstantExpression(value);
    },
        ...
        if (this.token.id == 'StringLiteral') {
            ...
            try {
                if (type == 'datetime') {
                    ...
                    if (date) {
                        typeExpression = new expressions.ConstantExpression(date);
                    }
                }
    ...
    _parseStringLiteral: function () {
        ...
        this._nextToken();
        return new expressions.ConstantExpression(s);
    },
    ...
    _createKeywords: function () {
        return {
            "true": new expressions.ConstantExpression(true),
            "false": new expressions.ConstantExpression(false),
            "null": new expressions.ConstantExpression(null),
...
var ODataParser = module.exports = core.defineClass(ctor, instanceMembers, classMembers);
