...
var instanceMembers = {
    format: function (query) {
        ...
        if (query.skip > 0 && query.take === undefined) {
            query.take = Number.MAX_SAFE_INTEGER + 1;
        }
    ...
    _formatOrderBy: function (query, defaultOrder) {
        ...
        var orderings = QueryParser.orderBy(query.ordering),
            order = '',
            self = this;
    ...
    _formatFilter: function (query, defaultFilter) {
        ...
        if (query.id !== undefined) {
            ...
            if (filterExpr) {
                filterExpr = new expressions.BinaryExpression(filterExpr, idFilterExpr, 'And');
            }
            else {
        ...
        if (this.tableMetadata.supportsSoftDelete && !query.includeDeleted) {
            ...
            if (filterExpr) {
                filterExpr = new expressions.BinaryExpression(filterExpr, deletedFilter, 'And');
            }
            else {
    ...
    _finalizeExpression: function (expr) {
        ...
        expr = convertTypes(expr, this.tableMetadata);
        return expr;
    },
    ...
    visitBinary: function (expr) {
        ...
        if (expr.expressionType == 'Modulo') {
            expr.left = new expressions.ConvertExpression('numeric', expr.left);
        }
        ...
        if ((left !== expr.left) || (right !== expr.right)) {
            return new expressions.BinaryExpression(left, right);
        }
    ...
    _formatMappedStringMember: function (instance, mappedMemberInfo, args) {
        ...
        if (functionName == 'substringof') {
            ...
            if (!isConstantOfType(args[0], 'string')) {
                args[0] = new expressions.ConvertExpression(helpers.getSqlType(''), args[0]);
            } else if (!isConstantOfType(args[1], 'string')) {
                args[1] = new expressions.ConvertExpression(helpers.getSqlType(''), args[1]);
            }
            var concat = new expressions.BinaryExpression(args[0], args[1], 'Add');
            this.visit(concat);
        }
...
var SqlFormatter = core.deriveClass(ExpressionVisitor, ctor, instanceMembers);

module.exports = function (query, schemaName, tableMetadata) {
    var formatter = new SqlFormatter(schemaName, tableMetadata);
