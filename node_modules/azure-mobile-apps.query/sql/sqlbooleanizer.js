...
(function (global) {
    ...
    var instanceMembers = {
        ...
        visitUnary: function (expr) {
            ...
            if (operand && expr.expressionType == 'Not') {
                ...
                return new UnaryExpression(ensureExpressionIsBoolean(operand), 'Not');
            }
    ...
    function rewriteBitComparison(left, right) {
        if (isBooleanExpression(left) && isBitConstantExpression(right)) {
            return (right.value === true) ? left : new UnaryExpression(left, 'Not');
        }
        else if (isBooleanExpression(right) && isBitConstantExpression(left)) {
            return (left.value === true) ? right : new UnaryExpression(right, 'Not');
        }
    ...
    function ensureExpressionIsBoolean(expr) {
        if (!isBooleanExpression(expr)) {
            return new BinaryExpression(expr, new ConstantExpression(true), 'Equal');
        }
        return expr;
    }
