...
var instanceMembers = {
    visitUnary: function (expr) {
        ...
        if (operand && expr.expressionType == 'Not') {
            ...
            return new expressions.UnaryExpression(ensureExpressionIsBoolean(operand), 'Not');
        }

        if (operand != expr.operand) {
            return new expressions.UnaryExpression(operand, expr.expressionType);
        }
    ...
    visitBinary: function (expr) {
        ...
        if (left != expr.left || right != expr.right) {
            return new expressions.BinaryExpression(left, right, expr.expressionType);
        }
...
function rewriteBitComparison(left, right) {
    if (isBooleanExpression(left) && isBitConstantExpression(right)) {
        return (right.value === true) ? left : new expressions.UnaryExpression(left, 'Not');
    }
    else if (isBooleanExpression(right) && isBitConstantExpression(left)) {
        return (left.value === true) ? right : new expressions.UnaryExpression(right, 'Not');
    }
...
function ensureExpressionIsBoolean(expr) {
    if (!isBooleanExpression(expr)) {
        return new expressions.BinaryExpression(expr, new expressions.ConstantExpression(true), 'Equal');
    }
    return expr;
}
...
var SqlBooleanizer = module.exports = core.deriveClass(ExpressionVisitor, null, instanceMembers);
