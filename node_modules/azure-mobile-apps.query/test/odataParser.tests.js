var expressions = require('../query/expressions'),
    ...
    QueryParser = require('../query/odataParser'),
    core = require('../core'),
...
describe('azure-mobile-apps.query.query.odataParser', function () {
    ...
    it("parse positive / negative numbers", function () {
        var result = new QueryParser('(val eq -1.5)').parse();
        equal(result.right.value, -1.5);

        result = new QueryParser('(val eq -1)').parse();
        equal(result.right.value, -1);

        result = new QueryParser('(val eq 1)').parse();
        equal(result.right.value, 1);

        result = new QueryParser('(val eq 1.5)').parse();
        equal(result.right.value, 1.5);
    });

    it("constant expression test", function () {
        var constExpr = new ConstantExpression('hello');
    ...
    it("basic expression tree test", function () {
        ...
        var left = new MemberExpression(p, 'user');
        var right = new ConstantExpression('mathewc');
        var binExpr = new BinaryExpression(left, right, 'Equal');
    ...
    it("numeric literal parsing", function () {
        ...
        for (var i = 0; i < testCases.length; i++) {
            ...
            var expr = new QueryParser(testCase.expr).parse();
    ...
    it("string replace - nested calls disallowed", function () {
        ...
        try {
            new QueryParser("replace(replace(name, 'x', 'y'), 'x', 'y') eq 'ApplePie'").parse();
        }
        catch (e) {
    ...
    it("string replace - replace arg must be constant with valid length", function () {
        ...
        try {
            ...
            new QueryParser("replace(name, 'x', nonConstant) eq 'ApplePie'").parse();
        }
        catch (e) {
            expectedExceptionCaught = true;
        ...
        try {
            ...
            new QueryParser("replace(name, 'x', 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx') eq 'ApplePie'").parse();
        }
        catch (e) {
            expectedExceptionCaught = true;
    ...
    it("parse error double quotes", function () {
        ...
        var expectedExceptionCaught = false;
        try {
            new QueryParser(expression).parse();
        }
        catch (e) {
            expectedExceptionCaught = true;
