...
describe('azure-mobile-apps.data.sql.statements', function () {
    ...
    describe('createIndex', function () {
        ...
        it('generates simple statement', function () {
            var statement = createIndex({ name: 'table' }, 'index1', ['foo']);
            expect(statement.sql).to.equal('CREATE INDEX [index1] ON [dbo].[table] ([foo])');
        });

        it('generates statement with multiple index columns', function () {
            var statement = createIndex({ name: 'table' }, 'index1', ['foo', 'bar', 'baz']);
            expect(statement.sql).to.equal('CREATE INDEX [index1] ON [dbo].[table] ([foo],[bar],[baz])');
        });
    })
});
